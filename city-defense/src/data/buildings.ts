...
const DEFS: Record<BuildingKind, BuildingDef> = {
  keep:      { kind: 'keep',      label: 'Keep',       cost: { wood: 0,   stone: 0,   gold: 0   }, buildTicks: 0,   maxHp: 1000, jobs: 0, validOn: ['grass', 'dirt'], blocking: true,  housing: 0 },
  house:     { kind: 'house',     label: 'House',      cost: { wood: 20,  stone: 0,   gold: 10  }, buildTicks: 200, maxHp: 80,   jobs: 0, validOn: ['grass', 'dirt'], blocking: true,  housing: 8 },
  ...
  warehouse: { kind: 'warehouse', label: 'Warehouse',  cost: { wood: 40,  stone: 10,  gold: 30  }, buildTicks: 300, maxHp: 120,  jobs: 2, validOn: ['grass', 'dirt'], blocking: true, foodStorage: 600 },
  ...
  tavern:    { kind: 'tavern',    label: 'Tavern',     cost: { wood: 30,  stone: 10,  gold: 40  }, buildTicks: 300, maxHp: 90,   jobs: 2, validOn: ['grass', 'dirt'], blocking: true, moraleBonus: 6 },
  chapel:    { kind: 'chapel',    label: 'Chapel',     cost: { wood: 20,  stone: 40,  gold: 60  }, buildTicks: 400, maxHp: 120,  jobs: 1, validOn: ['grass', 'dirt'], blocking: true, moraleBonus: 8 },
  market:    { kind: 'market',    label: 'Market',     cost: { wood: 35,  stone: 5,   gold: 80  }, buildTicks: 350, maxHp: 100,  jobs: 3, validOn: ['grass', 'dirt'], blocking: true, output: { resource: 'gold', perDay: 10 }, moraleBonus: 3 },
};
